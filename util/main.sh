...
export PROJECT_DIR=$(readlink -e $(dirname "${BASH_SOURCE[0]}")/..)
export INSTALL_DIR=$PROJECT_ROOT_DIR/opt
export RESULT_DIR=$PROJECT_DIR/result
...
export MACHINE_SPECIFIC_DIR=$PROJECT_DIR/util/machine-specific/$(hostname)
...
function CPUCountNuma {
  $PROJECT_DIR/util/get_cpus_in_numa.py
}
...
function bdmTaskset {
  local NUM_THREADS=$1
  shift
  taskset -c $($PROJECT_DIR/util/get_cpu_list.py $NUM_THREADS) "$@"
}
...
function bdmTasksetPhysicalCpus {
  local NUM_CPUS=$1
  shift
  taskset -c $($PROJECT_DIR/util/get_physical_cpu_list.py $NUM_CPUS true) "$@"
}
...
function benchmarkGpu {
  ...
  for i in $(seq 0 $(($BPE_REPEAT - 1))); do
    ...
    echo "Iteration: $i"
    echo ""
    taskset -c $($PROJECT_DIR/util/get_cpu_list.py $CPUS) $@ --inline-config="{ \"bdm::Param\": { \"preferred_gpu\": $BDM_GPU } }" >> $BENCH_LOG 2>&1
    echo ""
    echo "End   `date`"
    echo "---------------------------------------------------------------------"
    sleep 1
...
function collectMetadata {
  cp $PROJECT_DIR/util/reproduce/* $RESULT_DIR
  ...
  $PROJECT_DIR/util/system-info.sh &> $RESULT_METADATA_DIR/system-info
  $PROJECT_DIR/util/lines-of-code.sh &> $RESULT_METADATA_DIR/lines-of-code
}
